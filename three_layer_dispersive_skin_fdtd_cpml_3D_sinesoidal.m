...
boundary.air_buffer_number_of_cells_zp=20+40+40+40+40+200+200+200+200+200+200+200+200+200+200+200+200+200+200+200+200+200+200+200;
...
Hx=gpuArray(single((zeros(nxp1,nyp1,nzp1))));
Hy=gpuArray(single((zeros(nxp1,nyp1,nzp1))));
Hz=gpuArray(single((zeros(nxp1,nyp1,nzp1))));
Ex=gpuArray(single((zeros(nxp1,nyp1,nzp1))));
Ey=gpuArray(single((zeros(nxp1,nyp1,nzp1))));
Ez=gpuArray(single((zeros(nxp1,nyp1,nzp1))));
...
%% GPU parameters
AA1=(zeros(nx+1,1,nz+1,'logical'));
AA2=(zeros(nx+1,ny+1,1,'logical'));
AA4=(zeros(1,ny+1,nz+1,'logical'));
AA5=(zeros(nx+1,1,nz,'logical'));
AA6=(zeros(nx+1,ny+1,1,'logical'));
AA7=(zeros(nx,ny+1,+1,'logical'));
AA8=(zeros(1,ny+1,nz+1,'logical'));
AA9=(zeros(nx,+1,nz+1,'logical'));
AA10=(zeros(+1,ny+1,nz+1,'logical'));
...
Psi_hyx_xn=single(cat(3,Psi_hyx_xn,zeros(20,nyp1,1)));
Psi_hyx_xn=single(cat(1,Psi_hyx_xn,zeros(nxp1-20,nyp1,nzp1)));
Psi_hyx_xp=single(cat(3,Psi_hyx_xp,zeros(20,nyp1,1)));
Psi_hyx_xp=single(cat(1,zeros(nxp1-20,nyp1,nzp1),Psi_hyx_xp));
Psi_hzx_xn=single(cat(2,Psi_hzx_xn,zeros(20,1,nzp1)));
Psi_hzx_xn=single(cat(1,Psi_hzx_xn,zeros(nxp1-20,nyp1,nzp1)));
Psi_hzx_xp=single(cat(2,Psi_hzx_xp,zeros(20,1,nzp1)));
Psi_hzx_xp=single(cat(1,zeros(nxp1-20,nyp1,nzp1),Psi_hzx_xp));

CPsi_hyx_xn=single(cat(3,CPsi_hyx_xn,zeros(20,nyp1,1)));
CPsi_hyx_xn=single(cat(1,CPsi_hyx_xn,zeros(nxp1-20,nyp1,nzp1)));
% CPsi_hyx_xn=single(cat(1,zeros(1,nyp1,nzp1,'gpuArray'),CPsi_hyx_xn));


CPsi_hyx_xp=single(cat(3,CPsi_hyx_xp,zeros(20,nyp1,1)));
CPsi_hyx_xp=single(cat(1,zeros(nx-20,nyp1,nzp1),CPsi_hyx_xp));
CPsi_hyx_xp=single(cat(1,CPsi_hyx_xp,zeros(1,nyp1,nzp1)));

CPsi_hzx_xn=single(cat(2,CPsi_hzx_xn,zeros(20,1,nzp1)));
CPsi_hzx_xn=single(cat(1,CPsi_hzx_xn,zeros(nxp1-20,nyp1,nzp1)));
% CPsi_hzx_xn=single(cat(1,zeros(1,nyp1,nzp1,'gpuArray'),CPsi_hzx_xn));

CPsi_hzx_xp=single(cat(2,CPsi_hzx_xp,zeros(20,1,nzp1)));
CPsi_hzx_xp=single(cat(1,zeros(nx-20,nyp1,nzp1),CPsi_hzx_xp));
CPsi_hzx_xp=single(cat(1,CPsi_hzx_xp,zeros(1,nyp1,nzp1)));

cpml_b1_mx_xn=single(cpml_b_mx_xn'.*ones(20,ny,nzp1));
cpml_b1_mx_xn=single(cat(2,cpml_b1_mx_xn,zeros(20,1,nzp1)));
cpml_b1_mx_xn=single(cat(1,cpml_b1_mx_xn,zeros(nxp1-20,nyp1,nzp1)));
% cpml_b1_mx_xn=single(cat(1,zeros(1,nyp1,nzp1,'gpuArray'),cpml_b1_mx_xn));

cpml_b_mx_xn=single(cpml_b_mx_xn'.*ones(20,nyp1,nz));
cpml_b_mx_xn=single(cat(3,cpml_b_mx_xn,zeros(20,nyp1,1)));
cpml_b_mx_xn=single(cat(1,cpml_b_mx_xn,zeros(nxp1-20,nyp1,nzp1)));
% cpml_b_mx_xn=single(cat(1,zeros(1,nyp1,nzp1,'gpuArray'),cpml_b_mx_xn));

cpml_a1_mx_xn=single(cpml_a_mx_xn'.*ones(20,ny,nzp1));
cpml_a1_mx_xn=single(cat(2,cpml_a1_mx_xn,zeros(20,1,nzp1)));
cpml_a1_mx_xn=single(cat(1,cpml_a1_mx_xn,zeros(nxp1-20,nyp1,nzp1)));
% cpml_a1_mx_xn=single(cat(1,zeros(1,nyp1,nzp1,'gpuArray'),cpml_a1_mx_xn));

cpml_a_mx_xn=single(cpml_a_mx_xn'.*ones(20,nyp1,nz));
cpml_a_mx_xn=single(cat(3,cpml_a_mx_xn,zeros(20,nyp1,1)));
cpml_a_mx_xn=single(cat(1,cpml_a_mx_xn,zeros(nxp1-20,nyp1,nzp1)));
% cpml_a_mx_xn=single(cat(1,zeros(1,nyp1,nzp1,'gpuArray'),cpml_a_mx_xn));

cpml_b1_mx_xp=single(cpml_b_mx_xp'.*ones(20,ny,nzp1));
cpml_b1_mx_xp=single(cat(2,cpml_b1_mx_xp,zeros(20,1,nzp1)));
cpml_b1_mx_xp=single(cat(1,zeros(nx-20,nyp1,nzp1),cpml_b1_mx_xp));
cpml_b1_mx_xp=single(cat(1,cpml_b1_mx_xp,zeros(1,nyp1,nzp1)));

cpml_b_mx_xp=single(cpml_b_mx_xp'.*ones(20,nyp1,nz));
cpml_b_mx_xp=single(cat(3,cpml_b_mx_xp,zeros(20,nyp1,1)));
cpml_b_mx_xp=single(cat(1,zeros(nx-20,nyp1,nzp1),cpml_b_mx_xp));
cpml_b_mx_xp=single(cat(1,cpml_b_mx_xp,zeros(1,nyp1,nzp1)));

cpml_a1_mx_xp=single(cpml_a_mx_xp'.*ones(20,ny,nzp1));
cpml_a1_mx_xp=single(cat(2,cpml_a1_mx_xp,zeros(20,1,nzp1)));
cpml_a1_mx_xp=single(cat(1,zeros(nx-20,nyp1,nzp1),cpml_a1_mx_xp));
cpml_a1_mx_xp=single(cat(1,cpml_a1_mx_xp,zeros(1,nyp1,nzp1)));

cpml_a_mx_xp=single(cpml_a_mx_xp'.*ones(20,nyp1,nz));
cpml_a_mx_xp=single(cat(3,cpml_a_mx_xp,zeros(20,nyp1,1)));
cpml_a_mx_xp=single(cat(1,zeros(nx-20,nyp1,nzp1),cpml_a_mx_xp));
cpml_a_mx_xp=single(cat(1,cpml_a_mx_xp,zeros(1,nyp1,nzp1)));

Psi_hxy_yn=single(cat(3,zeros(nxp1,20,1),Psi_hxy_yn));
Psi_hxy_yn=single(cat(2,zeros(nxp1,nyp1-20,nzp1),Psi_hxy_yn));
Psi_hxy_yp=single(cat(3,Psi_hxy_yp,zeros(nxp1,20,1)));
Psi_hxy_yp=single(cat(2,Psi_hxy_yp,zeros(nxp1,nyp1-20,nzp1)));
Psi_hzy_yn=single(cat(1,zeros(1,20,nzp1),Psi_hzy_yn));
Psi_hzy_yn=single(cat(2,zeros(nxp1,nyp1-20,nzp1),Psi_hzy_yn));
Psi_hzy_yp=single(cat(1,Psi_hzy_yp,zeros(1,20,nzp1)));
Psi_hzy_yp=single(cat(2,Psi_hzy_yp,zeros(nxp1,nyp1-20,nzp1)));

CPsi_hxy_yn=single(cat(3,CPsi_hxy_yn,zeros(nxp1,20,1)));
CPsi_hxy_yn=single(cat(2,CPsi_hxy_yn,zeros(nxp1,nyp1-20,nzp1)));
% CPsi_hxy_yn=single(cat(2,zeros(nxp1,1,nzp1,'gpuArray'),CPsi_hxy_yn));

CPsi_hxy_yp=single(cat(3,CPsi_hxy_yp,zeros(nxp1,20,1)));
CPsi_hxy_yp=single(cat(2,zeros(nxp1,ny-20,nzp1),CPsi_hxy_yp));
CPsi_hxy_yp=single(cat(2,CPsi_hxy_yp,zeros(nxp1,1,nzp1')));

CPsi_hzy_yn=single(cat(1,CPsi_hzy_yn,zeros(1,20,nzp1)));
CPsi_hzy_yn=single(cat(2,CPsi_hzy_yn,zeros(nxp1,nyp1-20,nzp1)));
% CPsi_hzy_yn=single(cat(2,zeros(nxp1,1,nzp1,'gpuArray'),CPsi_hzy_yn));

CPsi_hzy_yp=single(cat(1,CPsi_hzy_yp,zeros(1,20,nzp1)));
CPsi_hzy_yp=single(cat(2,zeros(nxp1,ny-20,nzp1),CPsi_hzy_yp));
CPsi_hzy_yp=single(cat(2,CPsi_hzy_yp,zeros(nxp1,1,nzp1)));

cpml_b1_my_yn=single(cpml_b_my_yn.*ones(nx,20,nzp1));
cpml_b1_my_yn=single(cat(2,cpml_b1_my_yn,zeros(nx,nyp1-20,nzp1)));
% cpml_b1_my_yn=single(cat(2,zeros(nx,1,nzp1,'gpuArray'),cpml_b1_my_yn));
cpml_b1_my_yn=single(cat(1,cpml_b1_my_yn,zeros(1,nyp1,nzp1)));

cpml_b_my_yn=single(cpml_b_my_yn.*ones(nxp1,20,nz));
cpml_b_my_yn=single(cat(2,cpml_b_my_yn,zeros(nxp1,nyp1-20,nz)));
% cpml_b_my_yn=single(cat(2,zeros(nxp1,1,nz,'gpuArray'),cpml_b_my_yn));
cpml_b_my_yn=single(cat(3,cpml_b_my_yn,zeros(nxp1,nyp1,1)));

cpml_a1_my_yn=single(cpml_a_my_yn.*ones(nx,20,nzp1));
cpml_a1_my_yn=single(cat(2,cpml_a1_my_yn,zeros(nx,nyp1-20,nzp1)));
% cpml_a1_my_yn=single(cat(2,zeros(nx,1,nzp1,'gpuArray'),cpml_a1_my_yn));
cpml_a1_my_yn=single(cat(1,cpml_a1_my_yn,zeros(1,nyp1,nzp1)));

cpml_a_my_yn=single(cpml_a_my_yn.*ones(nxp1,20,nz));
cpml_a_my_yn=single(cat(2,cpml_a_my_yn,zeros(nxp1,nyp1-20,nz)));
% cpml_a_my_yn=single(cat(2,zeros(nxp1,1,nz,'gpuArray'),cpml_a_my_yn));
cpml_a_my_yn=single(cat(3,cpml_a_my_yn,zeros(nxp1,nyp1,1)));

cpml_b1_my_yp=single(cpml_b_my_yp.*ones(nx,20,nzp1));
cpml_b1_my_yp=single(cat(2,zeros(nx,ny-20,nzp1),cpml_b1_my_yp));
cpml_b1_my_yp=single(cat(2,cpml_b1_my_yp,zeros(nx,1,nzp1)));
cpml_b1_my_yp=single(cat(1,cpml_b1_my_yp,zeros(1,nyp1,nzp1)));

cpml_b_my_yp=single(cpml_b_my_yp.*ones(nxp1,20,nz));
cpml_b_my_yp=single(cat(2,zeros(nxp1,ny-20,nz),cpml_b_my_yp));
cpml_b_my_yp=single(cat(2,cpml_b_my_yp,zeros(nxp1,1,nz)));
cpml_b_my_yp=single(cat(3,cpml_b_my_yp,zeros(nxp1,nyp1,1)));

cpml_a1_my_yp=single(cpml_a_my_yp.*ones(nx,20,nzp1));
cpml_a1_my_yp=single(cat(2,zeros(nx,ny-20,nzp1),cpml_a1_my_yp));
cpml_a1_my_yp=single(cat(2,cpml_a1_my_yp,zeros(nx,1,nzp1)));
cpml_a1_my_yp=single(cat(1,cpml_a1_my_yp,zeros(1,nyp1,nzp1)));

cpml_a_my_yp=single(cpml_a_my_yp.*ones(nxp1,20,nz));
cpml_a_my_yp=single(cat(2,zeros(nxp1,ny-20,nz),cpml_a_my_yp));
cpml_a_my_yp=single(cat(2,cpml_a_my_yp,zeros(nxp1,1,nz)));
cpml_a_my_yp=single(cat(3,cpml_a_my_yp,zeros(nxp1,nyp1,1)));


Psi_hxz_zn=single(cat(2,Psi_hxz_zn,zeros(nxp1,1,20)));
Psi_hxz_zn=single(cat(3,Psi_hxz_zn,zeros(nxp1,nyp1,nzp1-20)));
Psi_hxz_zp=single(cat(2,Psi_hxz_zp,zeros(nxp1,1,20)));
Psi_hxz_zp=single(cat(3,zeros(nxp1,nyp1,nzp1-20),Psi_hxz_zp));
Psi_hyz_zn=single(cat(1,Psi_hyz_zn,zeros(1,nyp1,20)));
Psi_hyz_zn=single(cat(3,Psi_hyz_zn,zeros(nxp1,nyp1,nzp1-20)));
Psi_hyz_zp=single(cat(1,Psi_hyz_zp,zeros(1,nyp1,20)));
Psi_hyz_zp=single(cat(3,zeros(nxp1,nyp1,nzp1-20),Psi_hyz_zp));

CPsi_hxz_zn=single(cat(2,CPsi_hxz_zn,zeros(nxp1,1,20)));
CPsi_hxz_zn=single(cat(3,CPsi_hxz_zn,zeros(nxp1,nyp1,nzp1-20)));
% CPsi_hxz_zn=single(cat(3,zeros(nxp1,nyp1,1,'gpuArray'),CPsi_hxz_zn));

CPsi_hxz_zp=single(cat(2,CPsi_hxz_zp,zeros(nxp1,1,20)));
CPsi_hxz_zp=single(cat(3,zeros(nxp1,nyp1,nz-20),CPsi_hxz_zp));
CPsi_hxz_zp=single(cat(3,CPsi_hxz_zp,zeros(nxp1,nyp1,1)));

CPsi_hyz_zn=single(cat(1,CPsi_hyz_zn,zeros(1,nyp1,20)));
CPsi_hyz_zn=single(cat(3,CPsi_hyz_zn,zeros(nxp1,nyp1,nzp1-20)));
% CPsi_hyz_zn=single(cat(3,zeros(nxp1,nyp1,1,'gpuArray'),CPsi_hyz_zn));

CPsi_hyz_zp=single(cat(1,CPsi_hyz_zp,zeros(1,nyp1,20)));
CPsi_hyz_zp=single(cat(3,zeros(nxp1,nyp1,nz-20),CPsi_hyz_zp));
CPsi_hyz_zp=single(cat(3,CPsi_hyz_zp,zeros(nxp1,nyp1,1)));

cpml_b_mz_zn=reshape(cpml_b_mz_zn,1,1,20);
cpml_b1_mz_zn=single(cpml_b_mz_zn.*ones(nx,nyp1,20));
cpml_b1_mz_zn=single(cat(3,cpml_b1_mz_zn,zeros(nx,nyp1,nzp1-20)));
% cpml_b1_mz_zn=single(cat(3,zeros(nx,nyp1,1,'gpuArray'),cpml_b1_mz_zn));
cpml_b1_mz_zn=single(cat(1,cpml_b1_mz_zn,zeros(1,nyp1,nzp1)));

cpml_b_mz_zn=single(cpml_b_mz_zn.*ones(nxp1,ny,20));
cpml_b_mz_zn=single(cat(3,cpml_b_mz_zn,zeros(nxp1,ny,nzp1-20)));
% cpml_b_mz_zn=single(cat(3,zeros(nxp1,ny,1,'gpuArray'),cpml_b_mz_zn));
cpml_b_mz_zn=single(cat(2,cpml_b_mz_zn,zeros(nxp1,1,nzp1)));

cpml_a_mz_zn=reshape(cpml_a_mz_zn,1,1,20);
cpml_a1_mz_zn=single(cpml_a_mz_zn.*ones(nx,nyp1,20));
cpml_a1_mz_zn=single(cat(3,cpml_a1_mz_zn,zeros(nx,nyp1,nzp1-20)));
% cpml_a1_mz_zn=single(cat(3,zeros(nx,nyp1,1,'gpuArray'),cpml_a1_mz_zn));
cpml_a1_mz_zn=single(cat(1,cpml_a1_mz_zn,zeros(1,nyp1,nzp1)));

cpml_a_mz_zn=single(cpml_a_mz_zn.*ones(nxp1,ny,20));
cpml_a_mz_zn=single(cat(3,cpml_a_mz_zn,zeros(nxp1,ny,nzp1-20)));
% cpml_a_mz_zn=single(cat(3,zeros(nxp1,ny,1,'gpuArray'),cpml_a_mz_zn));
cpml_a_mz_zn=single(cat(2,cpml_a_mz_zn,zeros(nxp1,1,nzp1)));

cpml_b_mz_zp=reshape(cpml_b_mz_zp,1,1,20);
cpml_b1_mz_zp=single(cpml_b_mz_zp.*ones(nx,nyp1,20));
cpml_b1_mz_zp=single(cat(3,zeros(nx,nyp1,nz-20),cpml_b1_mz_zp));
cpml_b1_mz_zp=single(cat(3,cpml_b1_mz_zp,zeros(nx,nyp1,1)));
cpml_b1_mz_zp=cat(1,cpml_b1_mz_zp,zeros(1,nyp1,nzp1));

cpml_b_mz_zp=single(cpml_b_mz_zp.*ones(nxp1,ny,20));
cpml_b_mz_zp=single(cat(3,zeros(nxp1,ny,nz-20),cpml_b_mz_zp));
cpml_b_mz_zp=single(cat(3,cpml_b_mz_zp,zeros(nxp1,ny,1)));
cpml_b_mz_zp=single(cat(2,cpml_b_mz_zp,zeros(nxp1,1,nzp1)));

cpml_a_mz_zp=reshape(cpml_a_mz_zp,1,1,20);
cpml_a1_mz_zp=single(cpml_a_mz_zp.*ones(nx,nyp1,20));
cpml_a1_mz_zp=single(cat(3,zeros(nx,nyp1,nz-20),cpml_a1_mz_zp));
cpml_a1_mz_zp=single(cat(3,cpml_a1_mz_zp,zeros(nx,nyp1,1)));
cpml_a1_mz_zp=single(cat(1,cpml_a1_mz_zp,zeros(1,nyp1,nzp1)));

cpml_a_mz_zp=single(cpml_a_mz_zp.*ones(nxp1,ny,20));
cpml_a_mz_zp=single(cat(3,zeros(nxp1,ny,nz-20),cpml_a_mz_zp));
cpml_a_mz_zp=single(cat(3,cpml_a_mz_zp,zeros(nxp1,ny,1)));
cpml_a_mz_zp=single(cat(2,cpml_a_mz_zp,zeros(nxp1,1,nzp1)));
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Psi_eyx_xn=single(cat(2,Psi_eyx_xn,zeros(20,1,nzp1)));
Psi_eyx_xn=single(cat(1,Psi_eyx_xn,zeros(nxp1-20,nyp1,nzp1)));
Psi_eyx_xp=single(cat(2,Psi_eyx_xp,zeros(20,1,nzp1)));
Psi_eyx_xp=single(cat(1,zeros(nxp1-20,nyp1,nzp1),Psi_eyx_xp));
Psi_ezx_xn=single(cat(3,Psi_ezx_xn,zeros(20,nyp1,1)));
Psi_ezx_xn=single(cat(1,Psi_ezx_xn,zeros(nxp1-20,nyp1,nzp1)));
Psi_ezx_xp=single(cat(3,Psi_ezx_xp,zeros(20,nyp1,1)));
Psi_ezx_xp=single(cat(1,zeros(nxp1-20,nyp1,nzp1),Psi_ezx_xp));

CPsi_eyx_xn=single(cat(2,CPsi_eyx_xn,zeros(20,1,nzp1)));
CPsi_eyx_xn=single(cat(1,CPsi_eyx_xn,zeros(nx-20,nyp1,nzp1)));
CPsi_eyx_xn=single(cat(1,zeros(1,nyp1,nzp1),CPsi_eyx_xn));

CPsi_eyx_xp=single(cat(2,CPsi_eyx_xp,zeros(20,1,nzp1)));
CPsi_eyx_xp=single(cat(1,zeros(nx-20,nyp1,nzp1),CPsi_eyx_xp));
CPsi_eyx_xp=single(cat(1,CPsi_eyx_xp,zeros(1,nyp1,nzp1)));

CPsi_ezx_xn=single(cat(3,CPsi_ezx_xn,zeros(20,nyp1,1)));
CPsi_ezx_xn=single(cat(1,CPsi_ezx_xn,zeros(nx-20,nyp1,nzp1)));
CPsi_ezx_xn=single(cat(1,zeros(1,nyp1,nzp1),CPsi_ezx_xn));

CPsi_ezx_xp=single(cat(3,CPsi_ezx_xp,zeros(20,nyp1,1)));
CPsi_ezx_xp=single(cat(1,zeros(nx-20,nyp1,nzp1),CPsi_ezx_xp));
CPsi_ezx_xp=single(cat(1,CPsi_ezx_xp,zeros(1,nyp1,nzp1)));

cpml_b1_ex_xn=cpml_b_ex_xn'.*ones(20,nyp1,nz);
cpml_b1_ex_xn=single(cat(3,cpml_b1_ex_xn,zeros(20,nyp1,1)));
cpml_b1_ex_xn=single(cat(1,cpml_b1_ex_xn,zeros(nx-20,nyp1,nzp1)));
cpml_b1_ex_xn=single(cat(1,zeros(1,nyp1,nzp1),cpml_b1_ex_xn));

cpml_b_ex_xn=single(cpml_b_ex_xn'.*ones(20,ny,nzp1));
cpml_b_ex_xn=single(cat(2,cpml_b_ex_xn,zeros(20,1,nzp1)));
cpml_b_ex_xn=single(cat(1,cpml_b_ex_xn,zeros(nx-20,nyp1,nzp1)));
cpml_b_ex_xn=single(cat(1,zeros(1,nyp1,nzp1),cpml_b_ex_xn));

cpml_a1_ex_xn=single(cpml_a_ex_xn'.*ones(20,nyp1,nz));
cpml_a1_ex_xn=single(cat(3,cpml_a1_ex_xn,zeros(20,nyp1,1)));
cpml_a1_ex_xn=single(cat(1,cpml_a1_ex_xn,zeros(nx-20,nyp1,nzp1)));
cpml_a1_ex_xn=single(cat(1,zeros(1,nyp1,nzp1),cpml_a1_ex_xn));

cpml_a_ex_xn=single(cpml_a_ex_xn'.*ones(20,ny,nzp1));
cpml_a_ex_xn=single(cat(2,cpml_a_ex_xn,zeros(20,1,nzp1)));
cpml_a_ex_xn=single(cat(1,cpml_a_ex_xn,zeros(nx-20,nyp1,nzp1)));
cpml_a_ex_xn=single(cat(1,zeros(1,nyp1,nzp1),cpml_a_ex_xn));

cpml_b1_ex_xp=single(cpml_b_ex_xp'.*ones(20,nyp1,nz));
cpml_b1_ex_xp=single(cat(3,cpml_b1_ex_xp,zeros(20,nyp1,1)));
cpml_b1_ex_xp=single(cat(1,zeros(nx-20,nyp1,nzp1),cpml_b1_ex_xp));
cpml_b1_ex_xp=single(cat(1,cpml_b1_ex_xp,zeros(1,nyp1,nzp1)));

cpml_b_ex_xp=single(cpml_b_ex_xp'.*ones(20,ny,nzp1));
cpml_b_ex_xp=single(cat(2,cpml_b_ex_xp,zeros(20,1,nzp1)));
cpml_b_ex_xp=single(cat(1,zeros(nx-20,nyp1,nzp1),cpml_b_ex_xp));
cpml_b_ex_xp=single(cat(1,cpml_b_ex_xp,zeros(1,nyp1,nzp1)));

cpml_a1_ex_xp=single(cpml_a_ex_xp'.*ones(20,nyp1,nz));
cpml_a1_ex_xp=single(cat(3,cpml_a1_ex_xp,zeros(20,nyp1,1)));
cpml_a1_ex_xp=single(cat(1,zeros(nx-20,nyp1,nzp1),cpml_a1_ex_xp));
cpml_a1_ex_xp=single(cat(1,cpml_a1_ex_xp,zeros(1,nyp1,nzp1)));

cpml_a_ex_xp=single(cpml_a_ex_xp'.*ones(20,ny,nzp1));
cpml_a_ex_xp=single(cat(2,cpml_a_ex_xp,zeros(20,1,nzp1)));
cpml_a_ex_xp=single(cat(1,zeros(nx-20,nyp1,nzp1),cpml_a_ex_xp));
cpml_a_ex_xp=single(cat(1,cpml_a_ex_xp,zeros(1,nyp1,nzp1)));

Psi_exy_yn=single(cat(1,zeros(1,20,nzp1),Psi_exy_yn));
Psi_exy_yn=single(cat(2,zeros(nxp1,nyp1-20,nzp1),Psi_exy_yn));
Psi_exy_yp=single(cat(1,Psi_exy_yp,zeros(1,20,nzp1)));
Psi_exy_yp=single(cat(2,Psi_exy_yp,zeros(nxp1,nyp1-20,nzp1)));
Psi_ezy_yn=single(cat(3,zeros(nxp1,20,1),Psi_ezy_yn));
Psi_ezy_yn=single(cat(2,zeros(nxp1,nyp1-20,nzp1),Psi_ezy_yn));
Psi_ezy_yp=single(cat(3,Psi_ezy_yp,zeros(nxp1,20,1)));
Psi_ezy_yp=single(cat(2,Psi_ezy_yp,zeros(nxp1,nyp1-20,nzp1)));

CPsi_exy_yn=single(cat(1,CPsi_exy_yn,zeros(1,20,nzp1)));
CPsi_exy_yn=single(cat(2,CPsi_exy_yn,zeros(nxp1,ny-20,nzp1)));
CPsi_exy_yn=single(cat(2,zeros(nxp1,1,nzp1),CPsi_exy_yn));

CPsi_exy_yp=single(cat(1,CPsi_exy_yp,zeros(1,20,nzp1)));
CPsi_exy_yp=single(cat(2,zeros(nxp1,ny-20,nzp1),CPsi_exy_yp));
CPsi_exy_yp=single(cat(2,CPsi_exy_yp,zeros(nxp1,1,nzp1)));

CPsi_ezy_yn=single(cat(3,CPsi_ezy_yn,zeros(nxp1,20,1)));
CPsi_ezy_yn=single(cat(2,CPsi_ezy_yn,zeros(nxp1,ny-20,nzp1)));
CPsi_ezy_yn=single(cat(2,zeros(nxp1,1,nzp1),CPsi_ezy_yn));

CPsi_ezy_yp=single(cat(3,CPsi_ezy_yp,zeros(nxp1,20,1)));
CPsi_ezy_yp=single(cat(2,zeros(nxp1,ny-20,nzp1),CPsi_ezy_yp));
CPsi_ezy_yp=single(cat(2,CPsi_ezy_yp,zeros(nxp1,1,nzp1)));

cpml_b1_ey_yn=single(cpml_b_ey_yn.*ones(nxp1,20,nz));
cpml_b1_ey_yn=single(cat(2,cpml_b1_ey_yn,zeros(nxp1,ny-20,nz)));
cpml_b1_ey_yn=single(cat(2,zeros(nxp1,1,nz),cpml_b1_ey_yn));
cpml_b1_ey_yn=single(cat(3,cpml_b1_ey_yn,zeros(nxp1,nyp1,1)));

cpml_b_ey_yn=single(cpml_b_ey_yn.*ones(nx,20,nzp1));
cpml_b_ey_yn=single(cat(2,cpml_b_ey_yn,zeros(nx,ny-20,nzp1)));
cpml_b_ey_yn=single(cat(2,zeros(nx,1,nzp1),cpml_b_ey_yn));
cpml_b_ey_yn=single(cat(1,cpml_b_ey_yn,zeros(1,nyp1,nzp1)));

cpml_a1_ey_yn=single(cpml_a_ey_yn.*ones(nxp1,20,nz));
cpml_a1_ey_yn=single(cat(2,cpml_a1_ey_yn,zeros(nxp1,ny-20,nz)));
cpml_a1_ey_yn=single(cat(2,zeros(nxp1,1,nz),cpml_a1_ey_yn));
cpml_a1_ey_yn=single(cat(3,cpml_a1_ey_yn,zeros(nxp1,nyp1,1)));

cpml_a_ey_yn=single(cpml_a_ey_yn.*ones(nx,20,nzp1));
cpml_a_ey_yn=single(cat(2,cpml_a_ey_yn,zeros(nx,ny-20,nzp1)));
cpml_a_ey_yn=single(cat(2,zeros(nx,1,nzp1),cpml_a_ey_yn));
cpml_a_ey_yn=single(cat(1,cpml_a_ey_yn,zeros(1,nyp1,nzp1)));

cpml_b1_ey_yp=single(cpml_b_ey_yp.*ones(nxp1,20,nz));
cpml_b1_ey_yp=single(cat(2,zeros(nxp1,ny-20,nz),cpml_b1_ey_yp));
cpml_b1_ey_yp=single(cat(2,cpml_b1_ey_yp,zeros(nxp1,1,nz)));
cpml_b1_ey_yp=single(cat(3,cpml_b1_ey_yp,zeros(nxp1,nyp1,1)));

cpml_b_ey_yp=single(cpml_b_ey_yp.*ones(nx,20,nzp1));
cpml_b_ey_yp=single(cat(2,zeros(nx,ny-20,nzp1),cpml_b_ey_yp));
cpml_b_ey_yp=single(cat(2,cpml_b_ey_yp,zeros(nx,1,nzp1)));
cpml_b_ey_yp=single(cat(1,cpml_b_ey_yp,zeros(1,nyp1,nzp1)));

cpml_a1_ey_yp=single(cpml_a_ey_yp.*ones(nxp1,20,nz));
cpml_a1_ey_yp=single(cat(2,zeros(nxp1,ny-20,nz),cpml_a1_ey_yp));
cpml_a1_ey_yp=single(cat(2,cpml_a1_ey_yp,zeros(nxp1,1,nz)));
cpml_a1_ey_yp=single(cat(3,cpml_a1_ey_yp,zeros(nxp1,nyp1,1)));

cpml_a_ey_yp=single(cpml_a_ey_yp.*ones(nx,20,nzp1));
cpml_a_ey_yp=single(cat(2,zeros(nx,ny-20,nzp1),cpml_a_ey_yp));
cpml_a_ey_yp=single(cat(2,cpml_a_ey_yp,zeros(nx,1,nzp1)));
cpml_a_ey_yp=single(cat(1,cpml_a_ey_yp,zeros(1,nyp1,nzp1)));

Psi_exz_zn=single(cat(1,Psi_exz_zn,zeros(1,nyp1,20)));
Psi_exz_zn=single(cat(3,Psi_exz_zn,zeros(nxp1,nyp1,nzp1-20)));
Psi_exz_zp=single(cat(1,Psi_exz_zp,zeros(1,nyp1,20)));
Psi_exz_zp=single(cat(3,zeros(nxp1,nyp1,nzp1-20),Psi_exz_zp));
Psi_eyz_zn=single(cat(2,Psi_eyz_zn,zeros(nxp1,1,20)));
Psi_eyz_zn=single(cat(3,Psi_eyz_zn,zeros(nxp1,nyp1,nzp1-20)));
Psi_eyz_zp=single(cat(2,Psi_eyz_zp,zeros(nxp1,1,20)));
Psi_eyz_zp=single(cat(3,zeros(nxp1,nyp1,nzp1-20),Psi_eyz_zp));

CPsi_exz_zn=single(cat(1,CPsi_exz_zn,zeros(1,nyp1,20)));
CPsi_exz_zn=single(cat(3,CPsi_exz_zn,zeros(nxp1,nyp1,nz-20)));
CPsi_exz_zn=single(cat(3,zeros(nxp1,nyp1,1),CPsi_exz_zn));

CPsi_exz_zp=single(cat(1,CPsi_exz_zp,zeros(1,nyp1,20)));
CPsi_exz_zp=single(cat(3,zeros(nxp1,nyp1,nz-20),CPsi_exz_zp));
CPsi_exz_zp=single(cat(3,CPsi_exz_zp,zeros(nxp1,nyp1,1)));

CPsi_eyz_zn=single(cat(2,CPsi_eyz_zn,zeros(nxp1,1,20)));
CPsi_eyz_zn=single(cat(3,CPsi_eyz_zn,zeros(nxp1,nyp1,nz-20)));
CPsi_eyz_zn=single(cat(3,zeros(nxp1,nyp1,1),CPsi_eyz_zn));

CPsi_eyz_zp=single(cat(2,CPsi_eyz_zp,zeros(nxp1,1,20)));
CPsi_eyz_zp=single(cat(3,zeros(nxp1,nyp1,nz-20),CPsi_eyz_zp));
CPsi_eyz_zp=single(cat(3,CPsi_eyz_zp,zeros(nxp1,nyp1,1)));

cpml_b_ez_zn=reshape(cpml_b_ez_zn,1,1,20);

cpml_b1_ez_zn=single(cpml_b_ez_zn.*ones(nxp1,ny,20));
cpml_b1_ez_zn=single(cat(3,cpml_b1_ez_zn,zeros(nxp1,ny,nz-20)));
cpml_b1_ez_zn=single(cat(3,zeros(nxp1,ny,1),cpml_b1_ez_zn));
cpml_b1_ez_zn=single(cat(2,cpml_b1_ez_zn,zeros(nxp1,1,nzp1)));


cpml_b_ez_zn=single(cpml_b_ez_zn.*ones(nx,nyp1,20));
cpml_b_ez_zn=single(cat(3,cpml_b_ez_zn,zeros(nx,nyp1,nz-20)));
cpml_b_ez_zn=single(cat(3,zeros(nx,nyp1,1),cpml_b_ez_zn));
cpml_b_ez_zn=single(cat(1,cpml_b_ez_zn,zeros(1,nyp1,nzp1)));

cpml_a_ez_zn=reshape(cpml_a_ez_zn,1,1,20);

cpml_a1_ez_zn=single(cpml_a_ez_zn.*ones(nxp1,ny,20));
cpml_a1_ez_zn=single(cat(3,cpml_a1_ez_zn,zeros(nxp1,ny,nz-20)));
cpml_a1_ez_zn=single(cat(3,zeros(nxp1,ny,1),cpml_a1_ez_zn));
cpml_a1_ez_zn=single(cat(2,cpml_a1_ez_zn,zeros(nxp1,1,nzp1)));

cpml_a_ez_zn=single(cpml_a_ez_zn.*ones(nx,nyp1,20));
cpml_a_ez_zn=single(cat(3,cpml_a_ez_zn,zeros(nx,nyp1,nz-20)));
cpml_a_ez_zn=single(cat(3,zeros(nx,nyp1,1),cpml_a_ez_zn));
cpml_a_ez_zn=single(cat(1,cpml_a_ez_zn,zeros(1,nyp1,nzp1)));

cpml_b_ez_zp=reshape(cpml_b_ez_zp,1,1,20);

cpml_b1_ez_zp=single(cpml_b_ez_zp.*ones(nxp1,ny,20));
cpml_b1_ez_zp=single(cat(3,zeros(nxp1,ny,nz-20),cpml_b1_ez_zp));
cpml_b1_ez_zp=single(cat(3,cpml_b1_ez_zp,zeros(nxp1,ny,1)));
cpml_b1_ez_zp=single(cat(2,cpml_b1_ez_zp,zeros(nxp1,1,nzp1)));

cpml_b_ez_zp=single(cpml_b_ez_zp.*ones(nx,nyp1,20));
cpml_b_ez_zp=single(cat(3,zeros(nx,nyp1,nz-20),cpml_b_ez_zp));
cpml_b_ez_zp=single(cat(3,cpml_b_ez_zp,zeros(nx,nyp1,1)));
cpml_b_ez_zp=single(cat(1,cpml_b_ez_zp,zeros(1,nyp1,nzp1)));

cpml_a_ez_zp=reshape(cpml_a_ez_zp,1,1,20);

cpml_a1_ez_zp=single(cpml_a_ez_zp.*ones(nxp1,ny,20));
cpml_a1_ez_zp=single(cat(3,zeros(nxp1,ny,nz-20),cpml_a1_ez_zp));
cpml_a1_ez_zp=single(cat(3,cpml_a1_ez_zp,zeros(nxp1,ny,1)));
cpml_a1_ez_zp=single(cat(2,cpml_a1_ez_zp,zeros(nxp1,1,nzp1)));

cpml_a_ez_zp=single(cpml_a_ez_zp.*ones(nx,nyp1,20));
cpml_a_ez_zp=single(cat(3,zeros(nx,nyp1,nz-20),cpml_a_ez_zp));
cpml_a_ez_zp=single(cat(3,cpml_a_ez_zp,zeros(nx,nyp1,1)));
cpml_a_ez_zp=single(cat(1,cpml_a_ez_zp,zeros(1,nyp1,nzp1)));
...
SH_1=(ones(nxp1,nyp1,nzp1,'logical','gpuArray'));
SH_2=(ones(nxp1,nyp1,nzp1,'logical','gpuArray'));
SH_3=(ones(nxp1,nyp1,nzp1,'logical','gpuArray'));

% SH_1=(ones(nxp1,nyp1,nzp1,'single','gpuArray'));
% SH_2=(ones(nxp1,nyp1,nzp1,'single','gpuArray'));
% SH_3=(ones(nxp1,nyp1,nzp1,'single','gpuArray'));
...
sh_2=(ones(nxp1,nyp1,nzp1,'logical','gpuArray'));
sh_3=(ones(nxp1,nyp1,nzp1,'logical','gpuArray'));
source=single(zeros(nxp1,nyp1,nzp1,'logical','gpuArray'));
source(:,:,n_cpml_zn+10)=1;
% sh_1=(ones(nxp1,nyp1,nzp1,'single','gpuArray'));
% sh_2=(ones(nxp1,nyp1,nzp1,'single','gpuArray'));
% sh_3=(ones(nxp1,nyp1,nzp1,'single','gpuArray'));
...
Ex_sample=single((zeros(nxp1,nyp1,nzp1)));
...
clearvars -except Hx Hy Hz Ex Ey Ez ...
    ...
    dx dy dz SH_1 SH_2 SH_3 n_cpml_zn nx ny nz zcoor source

    cpml_b_ex_xn=gather(cpml_b_ex_xn);
    cpml_b_ex_xp=gather(cpml_b_ex_xp);
    cpml_b_ey_yn=gather(cpml_b_ey_yn);
    cpml_b_ey_yp=gather(cpml_b_ey_yp);
    cpml_b_ez_zn=gather(cpml_b_ez_zn);
    cpml_b_ez_zp=gather(cpml_b_ez_zp);
    cpml_a_ex_xn=gather(cpml_a_ex_xn);
    cpml_a_ex_xp=gather(cpml_a_ex_xp);
    cpml_a_ey_yn=gather(cpml_a_ey_yn);
    cpml_a_ey_yp=gather(cpml_a_ey_yp);
    cpml_a_ez_zn=gather(cpml_a_ez_zn);
    cpml_a_ez_zp=gather(cpml_a_ez_zp);
    cpml_b_mx_xn=gather(cpml_b_mx_xn);
    cpml_b_mx_xp=gather(cpml_b_mx_xp);
    cpml_b_my_yn=gather(cpml_b_my_yn);
    cpml_b_my_yp=gather(cpml_b_my_yp);
    cpml_b_mz_zn=gather(cpml_b_mz_zn);
    cpml_b_mz_zp=gather(cpml_b_mz_zp);
    cpml_a_mx_xn=gather(cpml_a_mx_xn);
    cpml_a_mx_xp=gather(cpml_a_mx_xp);
    cpml_a_my_yn=gather(cpml_a_my_yn);
    cpml_a_my_yp=gather(cpml_a_my_yp);
    cpml_a_mz_zn=gather(cpml_a_mz_zn);
    cpml_a_mz_zp=gather(cpml_a_mz_zp);
    cpml_b1_ex_xn=gather(cpml_b1_ex_xn);
    cpml_b1_ex_xp=gather(cpml_b1_ex_xp);
    cpml_b1_ey_yn=gather(cpml_b1_ey_yn);
    cpml_b1_ey_yp=gather(cpml_b1_ey_yp);
    cpml_b1_ez_zn=gather(cpml_b1_ez_zn);
    cpml_b1_ez_zp=gather(cpml_b1_ez_zp);
    cpml_a1_ex_xn=gather(cpml_a1_ex_xn);
    cpml_a1_ex_xp=gather(cpml_a1_ex_xp);
    cpml_a1_ey_yn=gather(cpml_a1_ey_yn);
    cpml_a1_ey_yp=gather(cpml_a1_ey_yp);
    cpml_a1_ez_zn=gather(cpml_a1_ez_zn);
    cpml_a1_ez_zp=gather(cpml_a1_ez_zp);
    cpml_b1_mx_xn=gather(cpml_b1_mx_xn);
    cpml_b1_mx_xp=gather(cpml_b1_mx_xp);
    cpml_b1_my_yn=gather(cpml_b1_my_yn);
    cpml_b1_my_yp=gather(cpml_b1_my_yp);
    cpml_b1_mz_zn=gather(cpml_b1_mz_zn);
    cpml_b1_mz_zp=gather(cpml_b1_mz_zp);
    cpml_a1_mx_xn=gather(cpml_a1_mx_xn);
    cpml_a1_mx_xp=gather(cpml_a1_mx_xp);
    cpml_a1_my_yn=gather(cpml_a1_my_yn);
    cpml_a1_my_yp=gather(cpml_a1_my_yp);
    cpml_a1_mz_zn=gather(cpml_a1_mz_zn);
    cpml_a1_mz_zp=gather(cpml_a1_mz_zp);  

      CPsi_eyx_xn=gather(CPsi_eyx_xn); 
      CPsi_eyx_xp=gather(CPsi_eyx_xp); 
      CPsi_ezx_xn=gather(CPsi_ezx_xn); 
      CPsi_ezx_xp=gather(CPsi_ezx_xp); 
      CPsi_ezy_yn=gather(CPsi_ezy_yn); 
      CPsi_ezy_yp=gather(CPsi_ezy_yp); 
      CPsi_exy_yn=gather(CPsi_exy_yn); 
      CPsi_exy_yp=gather(CPsi_exy_yp); 
      CPsi_eyz_zn=gather(CPsi_eyz_zn); 
      CPsi_eyz_zp=gather(CPsi_eyz_zp); 
      CPsi_exz_zn=gather(CPsi_exz_zn); 
      CPsi_exz_zp=gather(CPsi_exz_zp); 
      
      CPsi_hyx_xn=gather(CPsi_hyx_xn); 
      CPsi_hyx_xp=gather(CPsi_hyx_xp); 
      CPsi_hzx_xn=gather(CPsi_hzx_xn); 
      CPsi_hzx_xp=gather(CPsi_hzx_xp); 
      CPsi_hzy_yn=gather(CPsi_hzy_yn); 
      CPsi_hzy_yp=gather(CPsi_hzy_yp); 
      CPsi_hxy_yn=gather(CPsi_hxy_yn); 
      CPsi_hxy_yp=gather(CPsi_hxy_yp); 
      CPsi_hyz_zn=gather(CPsi_hyz_zn); 
      CPsi_hyz_zp=gather(CPsi_hyz_zp); 
      CPsi_hxz_zn=gather(CPsi_hxz_zn); 
      CPsi_hxz_zp=gather(CPsi_hxz_zp);     
...
Psi_exz_zn,cpml_b_ez_zn,cpml_b1_ez_zn,cpml_a_ez_zn,cpml_a1_ez_zn,Psi_eyz_zn,CPsi_exz_zn,CPsi_eyz_zn,...
Psi_exz_zp,cpml_b_ez_zp,cpml_b1_ez_zp,cpml_a_ez_zp,cpml_a1_ez_zp,Psi_eyz_zp,CPsi_exz_zp,CPsi_eyz_zp,...
 dx,dy,dz,SH_1,SH_2,SH_3,omega,number,dt,source);  

% Ex(:,:,n_cpml_zn+61)=Ex(:,:,n_cpml_zn+61)+sin(omega.*number.*dt);

% if mod(number,1)==0||number==totalTimeStep
% close all
% 
% % Hx_sample=Hx;
% % Hy_sample=Hy;
...
% for i=1:nz+1
%     Ex_sample_2D(i)=Ex((nx+1)/2,(ny+1)/2,i) ;
% end
